pipeline {
    ...
    environment {
        EXPORT_PATH       = "/Users/agi00114/Desktop/JenKins文件夹/pack_aplha"
        ...
        UM_LOG            = "${env.UM_LOG ?: 'false'}"   // 是否修改 launchMode
...
- [扫码安装](${qrUrl})
        ...
        stage('修改 Android launchMode') {
            when { expression { return env.BUILD_ANDROID == "true" && env.UM_LOG == "true" } }
            steps {
                dir('facesong_flutter') {
                    echo "⚙️ UM_LOG=true, 修改 Android launchMode..."
                    sh '''
                        sed -i '' '/<activity/,/<\\/activity>/ s/android:launchMode=".*"/android:launchMode="singleTop"/' android/app/src/main/AndroidManifest.xml
                    '''
                }
            }
        }
        ...
        stage('构建 Android APK') {
            ...
            steps {
                dir('facesong_flutter') {
                    echo "🤖 构建 Android APK (alpha flavor)"
                    sh '''
